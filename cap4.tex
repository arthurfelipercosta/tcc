...
A arquitetura do sistema é apresentada a seguir em três diagramas, cada um detalhando um fluxo principal: O processamento de dados (Figura \ref{fig:fluxo_dados}), a interação com a interface web (Figura \ref{fig:fluxo_web}) e a comunicação com o assistente conversacional (Figura \ref{fig:fluxo_whats}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Imagens/fig_diagrama_1.png}
    \caption{Fluxo de obtenção e processamento de dados.}
    \label{fig:fluxo_dados}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imagens/fig_diagrama_2.png}
    \caption{Arquitetura de interação da interface web.}
    \label{fig:fluxo_web}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imagens/fig_diagrama_3.png}
    \caption{Arquitetura de comunicação do assistente conversacional.}
    \label{fig:fluxo_mobile}
\end{figure}

Os principais componentes desta arquitetura são as fontes de dados externas, que incluem o dataset do PBEV e a FIPE API. A camada de processamento de dados, operada pelo Jupyter Notebook (fix.ipynb) com a biblioteca Pandas, responsáveis pela limpeza e padronização. O backend, implementado com Flask (api.py), servindo como um ponto de acesso central para os dados. E por fim, as interfaces com o usuário, que são compostas por um frontend web (HTML, CSS e JavaScript) e um bot para WhatsApp, que utiliza N8N e Twilio ({n8n\_bot.py}) para gerenciar as interações.
...
A base de dados PBEV, originalmente em formato PDF, foi convertida para CSV utilizando o serviço online Convertio \citep{CONVERTIO}. Este processo é crítico para viabilizar a manipulação programática dos dados. O resultado é um arquivo CSV ({dados\_corrigidos.csv}) que servirá como entrada para as próximas etapas.

\subsection{Limpeza e Otimização com Pandas no Jupyter Notebook}
O arquivo CSV do PBEV foi submetido a um rigoroso processo de limpeza e otimização. Este processo foi realizado no ambiente Jupyter Notebook (\textit{fix.ipynb}), utilizando a biblioteca Pandas \citep{PANDAS}. As principais operações incluíram:
\begin{itemize}
    \item Carregamento dos dados do arquivo {dados\_corrigidos.csv} para um DataFrame Pandas.
    ...
    \item Remoção de linhas totalmente duplicadas, gerando um arquivo final denominado '{dados\_corrigidos\_sem\_duplicatas.csv}', que é a versão otimizada da base de dados do PBEV.
\end{itemize}

O resultado final deste processo é o arquivo '{dados\_corrigidos\_sem\_duplicatas.csv}', uma base de dados otimizada e pronta para ser consumida pela API.

\section{Desenvolvimento do Backend (API)}
O backend da aplicação foi desenvolvido em Python utilizando o microframework Flask \citep{FLASK}, e seu código-fonte está no arquivo {api.py}. Este servidor atua como a fonte de dados para as interações do usuário, servindo dados e lógica de negócios para a aplicação web e para o assistente conversacional do WhatsApp.

Esta API é responsável por carregar e processar o dataset do PBEV, realizar consultas a APIs externas como a da FIPE \citep{FIPE}, e expor uma série de endpoints RESTful que atendem a diferentes necessidades:


\begin{itemize}
    \item \textbf{Para a Interface Web:} O principal endpoint consumido pelo frontend é o '/api/info {\_carro}'. Ele fornece os dados de preço de um veículo específico, permitindo que a interface web enriqueça as informações de especificações que são carregadas localmente.
    ...
    \begin{itemize}
        \item \textbf{Validação:} Rotas como '/api/validar\_marca' e '/api/versoes' para guiar o usuário na seleção de um veículo.
        \item \textbf{Comparação:} A rota '/api/comparativo', que realiza uma análise detalhada entre dois veículos e retorna mensagens formatadas para o WhatsApp.
        \item \textbf{Análise:} O endpoint '/api/top10', que permite consultas estatísticas sobre o catálogo de veículos.
        \item \textbf{Contexto para IA:} A rota {'/api/dados\_para\_ia'}, que estrutura os dados técnicos de forma otimizada para ser interpretada por modelos de linguagem.
    \end{itemize}
\end{itemize}
...
A interface web (index.html) contém todos os elementos de interação. Toda a lógica dinâmica e a interatividade são gerenciadas pelo arquivo script.js, e o fluxo de utilização da ferramenta se desenrola de forma contínua em uma única visualização, ilustrado pelas figuras a seguir.
...
Após a seleção de dois veículos, a área de comparativo é preenchida na mesma visualização. A Figura \ref{fig:interface_tabela} apresenta a tabela detalhada, exibindo os atributos dos veículos lado a lado, juntamente com um sistema de pontuação visual que destaca o desempenho em cada critério.
...
Para finalizar a análise, um resumo em linguagem natural é gerado e exibido de forma integrada, como ilustrado na Figura \ref{fig:interface_resumo}. Este resumo, apresentado na mesma interface, traduz os dados técnicos em frases de fácil compreensão, consolidando a informação para o usuário final.
...
Toda essa interatividade e dinamismo são controlados pelo arquivo script.js, que é responsável por:

\begin{itemize}
    \item Carregar os dados veiculares do arquivo {dados\_corrigidos\_sem\_duplicatas.csv}.
...
O fluxo de interação é orquestrado pela plataforma N8N \citep{N8N} e utiliza a API da Twilio \citep{TWILIO} como gateway de comunicação com o WhatsApp. O processo ocorre da seguinte forma:
...
\begin{enumerate}
    ...
    \item O N8N realiza uma requisição HTTP para a API Flask (api.py) para buscar os dados solicitados.
...
A centralização de toda a lógica de negócio em um único arquivo (api.py) garante a consistência dos dados em todas as plataformas e simplifica a manutenção e a escalabilidade do projeto.
